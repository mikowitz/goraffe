...
// example_rendering demonstrates rendering graphs to various output formats.
// This is not a testable example because rendering requires Graphviz
// and output sizes vary by version.
func example_rendering() {
	// Create a simple graph
	g := goraffe.NewGraph(goraffe.Directed, goraffe.WithGraphLabel("Workflow"))

	start := goraffe.NewNode("start", goraffe.WithLabel("Start"), goraffe.WithColor("green"))
	end := goraffe.NewNode("end", goraffe.WithLabel("End"), goraffe.WithColor("red"))
	...
	// Render to PNG file
	_ = g.RenderToFile(goraffe.PNG, "workflow.png")
	...
	_ = g.RenderToFile(goraffe.PNG, "workflow_neato.png",
		goraffe.WithLayout(goraffe.LayoutNeato))

	fmt.Println("Rendered successfully")
}

// exampleGraphvizVersion demonstrates checking the installed Graphviz version.
// This is not a testable example because version strings vary by system.
func exampleGraphvizVersion() {
	version, err := goraffe.GraphvizVersion()
	if err != nil {
		fmt.Println("Graphviz not installed")
		return
	}
	fmt.Printf("Graphviz version: %s\n", version)
}
