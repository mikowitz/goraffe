...
func TestGraph_DefaultNodeAttrs(t *testing.T) {
	...
	t.Run("returns configured default attributes", func(t *testing.T) {
		...
		g := NewGraph(WithDefaultNodeAttrs(WithBoxShape(), WithFontName("Arial")))
...
func TestWithDefaultNodeAttrs(t *testing.T) {
	...
	t.Run("options are applied in order", func(t *testing.T) {
		...
		g := NewGraph(WithDefaultNodeAttrs(
			WithBoxShape(),
			WithCircleShape(), // Later option should override
		))
...
func TestGraph_DefaultAttrs_BothNodeAndEdge(t *testing.T) {
	...
	t.Run("multiple calls to WithDefaultNodeAttrs only applies last one", func(t *testing.T) {
		...
		g := NewGraph(
			WithDefaultNodeAttrs(WithBoxShape()),
			WithDefaultNodeAttrs(WithCircleShape()),
		)
...
func TestWithDefaultNodeAttrs_UsingNodeAttributesStruct(t *testing.T) {
	...
	t.Run("only copies non-nil fields from NodeAttributes struct", func(t *testing.T) {
		asrt := assert.New(t)
		...
		template := NodeAttributes{
			shape: &shape,
			// label, color, fillColor, fontName, fontSize are nil
		}
		...
		asrt.Empty(attrs.Color(), "expected Color to remain empty (nil value not copied)")
		asrt.Empty(attrs.FillColor(), "expected FillColor to remain empty (nil value not copied)")
		asrt.Empty(attrs.FontName(), "expected FontName to remain empty (nil value not copied)")
		asrt.Zero(attrs.FontSize(), "expected FontSize to remain zero (nil value not copied)")
	})
...
func TestWithDefaultEdgeAttrs_UsingEdgeAttributesStruct(t *testing.T) {
	...
	t.Run("only copies non-nil fields from EdgeAttributes struct", func(t *testing.T) {
		asrt := assert.New(t)
		...
		template := EdgeAttributes{
			style: &style,
			// label, color, arrowHead, arrowTail, weight are nil
		}
		...
		asrt.Equal("Test Label", attrs.Label(), "expected Label to be set from option")
		asrt.Empty(attrs.Color(), "expected Color to remain empty (nil value not copied)")
		asrt.Empty(attrs.ArrowHead(), "expected ArrowHead to remain empty (nil value not copied)")
		asrt.Empty(attrs.ArrowTail(), "expected ArrowTail to remain empty (nil value not copied)")
		asrt.Zero(attrs.Weight(), "expected Weight to remain zero (nil value not copied)")
	})
